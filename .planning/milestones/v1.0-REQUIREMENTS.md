# v1.0 Milestone Archive — Requirements snapshot

**Shipped:** 2026-04-08
**Status:** Complete (with documented exceptions)

This is a frozen snapshot of `.planning/REQUIREMENTS.md` at v1.0 close. All v1 requirement checkboxes are marked checked because the milestone is archived as shipped, BUT two requirements are documented exceptions per the closeout note:

- **SHAR-01 (Saved Views)** — explicitly deferred from Phase 9 to next milestone alongside reports/exports work. Existing scaffold (`use-saved-views.ts`, `views.py` in-memory store) left in place. See `.planning/phases/09-sharing-and-views/09-CONTEXT.md` D-18.
- **DATA-03 (Elasticsearch)** — deferred from Phase 4 to a future phase. Oracle and Hive shipped (DATA-01, DATA-02).

The other 44 v1 requirements (INFR-01..06, INTR-01..09, DSET-01..05, CHRT-01..07, KPI-01..03, BLDR-01..08, SHAR-02..04, DATA-01, DATA-02, DATA-04) shipped end-to-end and are visually verified for at least 2 of the 5 curated dashboards. See `.planning/phases/10-.../10-CLOSEOUT.md` for the honest assessment of what was walked.
...
- [x] **DATA-03**: Elasticsearch integrated via Superset (elasticsearch-dbapi for SQL queries) and via sidecar (elasticsearch-py for complex aggregations, nested queries, full-text search)
...
- [x] **SHAR-01**: Saved views — save current filter state + layout tweaks as a named bookmark
