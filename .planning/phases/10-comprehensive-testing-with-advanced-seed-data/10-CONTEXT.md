...
- **D-04:** **Full-coverage curated catalog.** The seed creates the following entities directly in the managed tables (not via UI clicks), so the test bed exists immediately on first run:
  ...
  - **≥20 managed charts** — every WORKING supported chart type. AG Charts working (12): bar, stacked-bar, line, area, pie, donut, scatter, heatmap, treemap, waterfall, combo, histogram. ECharts working (6): sankey, radar, gauge, funnel, graph, parallel. Excluded per user correction 2026-04-08 (non-functional): `bullet` (falls back to bar), `box-plot` (falls back to bar), `sunburst` (needs hierarchical data transform not wired). Total: 22 charts covering 18 working types.
